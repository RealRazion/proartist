<template>
  <div class="guides wide">
    <header class="card header">
      <div>
        <h1>Plugin Guides</h1>
        <p class="muted">Tipps, Workflows und Best Practices vom Team.</p>
      </div>
      <button class="btn ghost" type="button" @click="loadGuides" :disabled="loading">
        {{ loading ? "Lade..." : "Aktualisieren" }}
      </button>
    </header>

    <section v-if="isTeam" class="card editor">
      <h2>Neuen Guide erstellen</h2>
      <form class="form" @submit.prevent="createGuide">
        <label>
          Titel
        ...
        <label>
          Inhalt
          <textarea class="input textarea" v-model.trim="form.body" placeholder="Schreibe den Guide hier..."></textarea>
        </label>
        <label class="toggle">
        ...
        <button class="btn" type="submit" :disabled="saving">
          {{ saving ? "Speichere..." : "Guide speichern" }}
        </button>
      </form>
    </section>
    ...
    <section class="card posts">
      ...
      <ul v-else>
        <li v-for="guide in guides" :key="guide.id">
          <div class="post-head">
            <h3>{{ guide.title }}</h3>
            <span class="badge" :class="{ draft: !guide.is_published }">
              {{ guide.is_published ? "Live" : "Entwurf" }}
            </span>
          </div>
          <p class="muted">{{ formatDate(guide.created_at) }} von {{ guide.author?.name || guide.author?.username }}</p>
          <img v-if="guide.image || guide.image_url" class="post-image" :src="guide.image || guide.image_url" :alt="guide.title" />
          <p>{{ guide.body }}</p>
          <div v-if="isTeam" class="actions">
            <button class="btn ghost tiny" type="button" @click="togglePublish(guide)" :disabled="savingIds.has(guide.id)">
      ...
      <p v-if="!loading && !guides.length" class="muted empty">Noch keine Guides vorhanden.</p>
    </section>
  </div>
</template>
...
async function loadGuides() {
  loading.value = true;
  try {
    const { data } = await api.get("plugin-guides/");
    guides.value = data;
  } catch (err) {
    console.error("Guides konnten nicht geladen werden", err);
    guides.value = [];
    showToast("Guides konnten nicht geladen werden", "error");
  } finally {
    loading.value = false;
  }
}
...
async function createGuide() {
  ...
  try {
    if (imageFile.value) {
      ...
      if (cropped) {
        payload.append("image", cropped, imageFile.value.name || "guide-image.jpg");
      } else {
        payload.append("image", imageFile.value, imageFile.value.name);
      }
      await api.post("plugin-guides/", payload, { headers: { "Content-Type": "multipart/form-data" } });
    } else {
      await api.post("plugin-guides/", form.value);
    }
    form.value = { title: "", body: "", is_published: true };
    clearImage();
    await loadGuides();
  } catch (err) {
    console.error("Guide konnte nicht gespeichert werden", err);
    showToast("Guide konnte nicht gespeichert werden", "error");
  } finally {
    saving.value = false;
  }
}
...
async function togglePublish(guide) {
  ...
  try {
    await api.post(`plugin-guides/${guide.id}/publish/`, { publish: !guide.is_published });
    await loadGuides();
  } catch (err) {
